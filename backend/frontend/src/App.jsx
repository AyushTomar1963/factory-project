...
//     try {
//       const response = await fetch("https://factory-project-1.onrender.com/api/log-inspection", {
//         method: "POST",
...
//           </div>
//           <button onClick={handleLogout} className="bg-red-50 text-red-600 hover:bg-red-100 font-bold py-2 px-4 rounded-lg transition-colors">
//             Logout Station
//           </button>
//         </div>
...
//               <h3 className="text-gray-500 font-bold text-sm mb-1">Total Inspections</h3>
//               <p className="text-4xl font-black text-gray-900">{stats.total_inspections}</p>
//             </div>
//             <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
//               <h3 className="text-gray-500 font-bold text-sm mb-1">Failed/Marginal</h3>
//               <p className="text-4xl font-black text-red-600">{stats.failed_inspections}</p>
//             </div>
//             <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
//               <h3 className="text-gray-500 font-bold text-sm mb-1">Overall Yield Rate</h3>
//               <p className={`text-4xl font-black ${stats.yield_rate >= 90 ? 'text-green-500' : 'text-yellow-500'}`}>
//                 {stats.yield_rate}%
//               </p>
//             </div>
//           </div>
...
//                     <td className="p-4 font-bold text-gray-900">{log["Part Number"] || "N/A"}</td>
//                     <td className="p-4 text-gray-600">{log.Stage || "N/A"}</td>
//                     <td className="p-4 text-gray-600">{log["Logged By"] || log.Worker || "Unknown"}</td>
//                     <td className="p-4">
//                       <span className={`px-3 py-1 rounded-full text-xs font-bold ${
//                         log.Status === "GREEN" ? "bg-green-100 text-green-700" :
//                         log.Status === "YELLOW" ? "bg-yellow-100 text-yellow-800" :
...
//                 {stats?.logs?.length === 0 && (
//                   <tr>
//                     <td colSpan="6" className="p-8 text-center text-gray-500 font-bold">No inspection logs found yet.</td>
//                   </tr>
//                 )}
//               </tbody>
//             </table>
  ...
  useEffect(() => {
    if (partNumber && sheetId && token) {
      ...
      fetch(`${API_BASE}/api/get-spec/${partNumber}?sheet_id=${sheetId}`, {
        ...
        .then(res => {
          if (!res.ok) throw new Error("Part not found or access denied.")
          return res.json()
        })
        .then(data => {
          setSpecData(data)
          const initialValues = {}
          if (data.parameters) {
            data.parameters.forEach(param => { initialValues[param] = "" })
          }
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAiReply("AI Error: " + data.detail)
      }
    } catch {
      setAiReply("Network error connecting to AI.")
    } finally {
      setIsAskingAi(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6 relative">
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8 mt-10 border border-gray-200">
        ...
        {specData && isAllRated && !overallStatus && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <button
                onClick={() => setOverallStatus("YELLOW")}
                disabled={isSubmitting}
        ...
        {/* YELLOW: AI Supervisor Chatbot */}
        {overallStatus === "YELLOW" && (
          <div className="mt-4 p-4 border-2 border-yellow-400 bg-yellow-50 rounded-xl">
            <h2 className="text-lg font-black text-yellow-800 mb-2">⚠️ Marginal Pass: AI Supervisor</h2>
            <p className="text-xs text-yellow-700 mb-4 font-bold">
              Before submitting, ask the AI what to check, or log your doubt.
            </p>
            ...
            {aiReply && (
              <div className="mb-4 p-3 bg-white border border-yellow-200 rounded-lg text-sm text-gray-800 shadow-inner whitespace-pre-wrap">
                <strong className="text-blue-600">AI Response:</strong><br/>
                {aiReply}
              </div>
            )}
            ...
            <div className="flex gap-2 mb-4">
              <input
                type="text"
                placeholder="E.g. The vibration is slightly high, what should I check?"
                value={chatMessage}
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 rounded-lg disabled:opacity-50 transition-colors text-sm"
              >
                {isAskingAi ? "..." : "Ask AI"}
              </button>
            </div>
            ...
            <div className="flex gap-4">
              <button
                onClick={() => { setOverallStatus(null); setRemark("") }}
              ...
              <button
                onClick={() => {
                  const finalRemark = aiReply
                    ? `Doubt: ${chatMessage} | AI Advice: ${aiReply}`
                    : chatMessage || "Marginal - escalated"
                  handleLogSubmission("YELLOW", finalRemark)
                }}
                disabled={isSubmitting}
                className="w-2/3 bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-bold py-3 rounded-lg disabled:opacity-50"
              >
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 border-t-4 border-t-green-500">
              ...
              <p className={`text-3xl font-black ${stats.yield_rate >= 90 ? 'text-green-600' : 'text-yellow-600'}`}>
                {stats.yield_rate}%
              </p>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 border-t-4 border-t-red-500">
              ...
              <p className="text-xl font-black text-gray-900 mt-2 truncate" title={stats.top_defect}>
                {stats.top_defect}
              </p>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 border-t-4 border-t-purple-500">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs font-bold text-gray-600">
                  <span>Stage 1</span><span className="text-red-500">{stats.stage_failures?.["Stage 1"] || 0}</span>
                </div>
                <div className="flex justify-between text-xs font-bold text-gray-600">
                  <span>Stage 2</span><span className="text-red-500">{stats.stage_failures?.["Stage 2"] || 0}</span>
                </div>
                <div className="flex justify-between text-xs font-bold text-gray-600">
                  <span>Stage 3</span><span className="text-red-500">{stats.stage_failures?.["Stage 3"] || 0}</span>
                </div>
              </div>
            </div>
          </div>

...
#     spreadsheet = gc.open_by_key("1eFMcq_pSOeOvz8-StfzFryOHN8Da9MsVL4nS_fGTAQE")
...
#         print("Headers missing or outdated! Re-injecting correct headers into Row 1...")
#         if first_row:
...
# # Helper: removes ALL whitespace including invisible chars
# def clean(s: str) -> str:
...
# # FIX: Uses index-based reading (no blank header issues)
# # FIX: Uses clean() to strip all invisible whitespace from codes
# # -----------------------------------------------------------------
...
#             # FIX: clean() strips spaces, tabs, invisible chars from both sides
#             sheet_code = clean(row[3])
#             incoming_code = clean(part_number)

#             if sheet_code == incoming_code:
#                 # Parameters are at fixed column indexes: 4, 7, 10, 13
#                 # (skipping the Tight/Loose/< > helper columns in between)
#                 raw_params = []
...
#                     "part_name": row[2].strip(),  # Column C = Item
#                     "group": row[1].strip(),       # Column B = Group
...
#                 raise HTTPException(
#                     status_code=400,
#                     detail="CRITICAL: Stage 1 must be passed GREEN before Stage 2!"
#                 )
...
#                 raise HTTPException(
#                     status_code=400,
#                     detail="CRITICAL: Stage 2 must be passed GREEN before Stage 3!"
#                 )
...
#         Part '{request.part_name}' failed at '{request.current_stage}'.
#         Recorded measurements: {measurements_str}
#         Worker's remark: '{request.worker_remark}'
        
#         1. Categorize the defect into 1-3 words (e.g., 'Dimensional Error', 'Physical Damage').
